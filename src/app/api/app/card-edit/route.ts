...
import { editCard } from "@/infrastructure/appwrite/runtime/product-commands.server";
...
export async function POST(request: NextRequest) {
  return runTrustedCommand(request, schema, async (context, input) => {
    const intentSeed = { cardId: String(input.cardId), name: String(input.name), type: input.type as "debit" | "credit", colorId: String(input.colorId) };
    return runWithCommandEnvelope(
      { commandType: "edit-card", commandId: String(input.commandId), intentSeed },
      () => editCard(context, String(input.cardId), { ...intentSeed, commandId: String(input.commandId) }),
    );
  });
}
