...
import { getSettlementPage } from "@/infrastructure/appwrite/runtime/product-reads.server";
...
export async function GET(request: NextRequest) {
  return runProductRead(request, async (context) => {
    ...
    return getSettlementPage(context, householdId(String(input.householdId)) as HouseholdId);
  });
}
