...
import { z } from "zod";
import { leaveHousehold } from "@/infrastructure/appwrite/runtime/product-commands.server";
...
export async function POST(request: NextRequest) {
  return runTrustedCommand(request, schema, async (context, input) => {
    return runWithCommandEnvelope({ commandType: "leave-household", commandId: String(input.commandId), intentSeed: {} }, () =>
      leaveHousehold(context));
  });
}
