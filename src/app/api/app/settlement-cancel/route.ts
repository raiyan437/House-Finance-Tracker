...
export async function POST(request: NextRequest) {
  return runTrustedCommand(request, settlementTransitionInput, (context, input) => {
    const intentSeed = { settlementId: String(input.settlementId), status: "cancelled" };
    return runWithCommandEnvelope(
      { commandType: "cancel-settlement", commandId: String(input.commandId), intentSeed },
      () => transitionSettlement(context, String(input.settlementId), "cancelled", String(input.commandId)),
    );
  });
}
