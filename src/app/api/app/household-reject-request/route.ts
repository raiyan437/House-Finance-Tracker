...
import { z } from "zod";
import { rejectJoinRequest } from "@/infrastructure/appwrite/runtime/product-commands.server";
...
export async function POST(request: NextRequest) {
  return runTrustedCommand(request, schema, async (context, input) => {
    ...
    return runWithCommandEnvelope({ commandType: "reject-join-request", commandId: String(input.commandId), intentSeed }, () =>
      rejectJoinRequest(context, String(input.joinRequestId)));
  });
}
