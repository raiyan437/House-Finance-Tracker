...
export async function POST(request: NextRequest) {
  return runTrustedCommand(request, settlementTransitionInput, (context, input) => {
    const intentSeed = { settlementId: String(input.settlementId), status: "confirmed" };
    return runWithCommandEnvelope(
      { commandType: "confirm-settlement", commandId: String(input.commandId), intentSeed },
      () => transitionSettlement(context, String(input.settlementId), "confirmed", String(input.commandId)),
    );
  });
}
