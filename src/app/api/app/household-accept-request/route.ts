...
import { z } from "zod";
import { acceptJoinRequest } from "@/infrastructure/appwrite/runtime/product-commands.server";
...
export async function POST(request: NextRequest) {
  return runTrustedCommand(request, schema, async (context, input) => {
    ...
    return runWithCommandEnvelope({ commandType: "accept-join-request", commandId: String(input.commandId), intentSeed }, () =>
      acceptJoinRequest(context, String(input.joinRequestId)));
  });
}
