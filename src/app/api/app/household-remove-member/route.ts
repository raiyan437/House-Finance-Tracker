...
import { z } from "zod";
import { removeMember } from "@/infrastructure/appwrite/runtime/product-commands.server";
...
export async function POST(request: NextRequest) {
  return runTrustedCommand(request, schema, async (context, input) => {
    ...
    return runWithCommandEnvelope({ commandType: "remove-member", commandId: String(input.commandId), intentSeed }, () =>
      removeMember(context, String(input.memberId)));
  });
}
