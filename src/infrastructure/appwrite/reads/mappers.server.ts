...
export function mapProfileDisplay(raw: unknown): ProfileDisplay {
  return malformed("profiles", () => {
    ...
    return Object.freeze({ userId: userId(rowId(raw)), displayName: value.displayName, createdAt: isoInstant(value.createdAt), updatedAt: isoInstant(value.updatedAt) });
  }) as ProfileDisplay;
}
...
export function mapHousehold(raw: unknown): Household {
  return malformed("households", () => {
    ...
    const result: Household = { householdId: householdId(rowId(raw)), name: value.name, code: value.code, createdAt: isoInstant(value.createdAt), updatedAt: isoInstant(value.updatedAt), ...(value.deletedAt && value.deletedByUserId ? { deletedAt: isoInstant(value.deletedAt), deletedByUserId: userId(value.deletedByUserId) } : {}) };
...
export function mapMembership(raw: unknown): MembershipSnapshot {
  return malformed("memberships", () => {
    ...
    isoInstant(value.joinedAt); isoInstant(value.statusChangedAt); if (value.leftAt) isoInstant(value.leftAt);
...
export function mapExpense(raw: unknown): Expense {
  return malformed("expenses", () => {
    ...
    const result: Expense = { expenseId: id, householdId: householdId(value.householdId), creatorId: userId(value.createdBy), payerId: userId(value.payerId), name: value.name, amount: positivePoisha(safeInteger(value.amountPoisha)), expenseDate: expenseDate(value.expenseDate), splitMethod: value.splitMethod, ...(percentages ? { percentageEntries: percentages } : {}), allocations, payment: value.paymentMethod === "cash" ? { method: "cash" } : { method: "card", cardReference: `private:${id}` }, revision: safeInteger(value.revision), createdAt: isoInstant(value.createdAt), updatedAt: isoInstant(value.updatedAt), ...(value.deletedAt && value.deletedByUserId ? { deletedAt: isoInstant(value.deletedAt), deletedByUserId: userId(value.deletedByUserId) } : {}) };
...
export function mapCard(raw: unknown): Card {
  return malformed("cards", () => {
    ...
    const result: Card = { cardId: cardId(rowId(raw)), ownerId: userId(value.ownerId), name: value.name, type: value.type, colorId: cardColorId(value.design), createdAt: isoInstant(value.createdAt), updatedAt: isoInstant(value.updatedAt), ...(value.archivedAt ? { archivedAt: isoInstant(value.archivedAt) } : {}) };
...
export function mapReceiptMetadata(raw: unknown): ReceiptMetadata {
  return malformed("receipt_metadata", () => {
    ...
    const result: ReceiptMetadata = { receiptId: receiptId(rowId(raw)), householdId: householdId(value.householdId), expenseId: expenseId(value.expenseId), createdByUserId: userId(value.uploaderId), mimeType: value.mimeType, sizeBytes: value.sizeBytes, createdAt: isoInstant(value.createdAt), contentStatus: value.contentState, ...(value.originalFilename ? { originalFilename: value.originalFilename } : {}), ...(value.contentState !== "available" && value.contentRemovedAt ? { contentRemovedAt: isoInstant(value.contentRemovedAt) } : {}), ...(value.contentRemovedByUserId ? { contentRemovedByUserId: userId(value.contentRemovedByUserId) } : {}) };
