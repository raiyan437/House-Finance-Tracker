...
export function deterministicSeedData() {
  ...
  const cards: readonly Card[] = [
    { cardId: cardId("card-raiyan-debit"), ownerId: SEEDED_USER_IDS.raiyan, name: "Daily Debit", type: "debit", color: "lime", createdAt, updatedAt: createdAt },
    { cardId: cardId("card-john-credit"), ownerId: SEEDED_USER_IDS.john, name: "John Credit", type: "credit", color: "blue", createdAt, updatedAt: createdAt },
  ];
  const expenseCash: Expense = { expenseId: expenseId("expense-groceries"), householdId: SEEDED_HOUSEHOLD_ID, creatorId: SEEDED_USER_IDS.raiyan, payerId: SEEDED_USER_IDS.raiyan, name: "Groceries", amount: positivePoisha(30000), expenseDate: expenseDate("2026-08-10"), splitMethod: "equal", allocations: [SEEDED_USER_IDS.john, SEEDED_USER_IDS.raiyan, SEEDED_USER_IDS.sarah].map((participantId) => ({ participantId, share: poisha(10000) })), payment: { method: "cash" }, createdAt, updatedAt: createdAt };
  const expenseCard: Expense = { expenseId: expenseId("expense-internet"), householdId: SEEDED_HOUSEHOLD_ID, creatorId: SEEDED_USER_IDS.john, payerId: SEEDED_USER_IDS.john, name: "Internet", amount: positivePoisha(15000), expenseDate: expenseDate("2026-08-11"), splitMethod: "equal", allocations: [SEEDED_USER_IDS.john, SEEDED_USER_IDS.sarah].map((participantId) => ({ participantId, share: poisha(7500) })), payment: { method: "card", cardReference: "private:expense-internet" }, createdAt, updatedAt: createdAt };
  const privateCard: ExpenseCardPrivateSnapshot = { expenseId: expenseCard.expenseId, ownerId: SEEDED_USER_IDS.john, cardId: cards[1].cardId, cardName: cards[1].name, cardType: cards[1].type, color: cards[1].color };
  ...
  const receipt: ReceiptMetadata = { receiptId: receiptId("receipt-groceries"), householdId: SEEDED_HOUSEHOLD_ID, expenseId: expenseCash.expenseId, createdByUserId: SEEDED_USER_IDS.raiyan, mimeType: "image/png", originalFilename: "groceries.png", sizeBytes: 8, createdAt };
  const receiptBytes = new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);
