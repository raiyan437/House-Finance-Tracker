...
- **REQUIREMENT CHANGE approved on 2026-08-30:** `profiles.displayName` remains the authoritative product Display Name and is editable from Profile as a mutation separate from password change. Input is a string, trimmed, and must be non-empty; there is no approved product maximum. Appwrite's 16,383-character column capacity is infrastructure-only. The command derives its actor exclusively from `hft_session`, uses `Profile.version` OCC plus actor/type/command-id/SHA-256 intent idempotency, and treats an unchanged trimmed value as a write-free, version-stable no-op. Appwrite Auth name is not authoritative.
