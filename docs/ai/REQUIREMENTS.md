...
Out of scope: recurring expenses, budgets, multiple currencies, multiple households per user, export, notifications, real banking/cards/payment processing, banking APIs, receipt OCR, and paid third-party services. Currency is BDT only. Initial development and dependencies must add no project cost.

## v1.3 Expense icons and comments

- **REQUIREMENT CHANGE approved on 2026-09-02:** every Expense has one semantic category from `internet`, `gas`, `groceries`, `food`, `entertainment`, `cigarettes`, `pets`, `repairs`, `housing`, or `others`. Presentation owns the single Lucide mapping; domain and persistence never store component names. New Expenses default to `others`, and a missing legacy value reads as `others` without backfill.
- The Create Expense category selector sits directly below Expense Name as one compact, visually icon-only horizontal radio row in the canonical category order. Semantic labels remain available through accessible names and focus/hover tooltips; selected state is not color-only. Glyphs are approximately 19px inside 44px targets. Narrow layouts scroll only the internal non-wrapping strip, never the page. Edit reuses the same visual selector without reorganizing unrelated fields.
- Category is editable non-financial metadata like Expense Name. It is excluded from the Expense financial fingerprint, balance/allocation/recommendation/Settlement calculations, and confirmed-history comparison, so an otherwise-authorized icon-only edit remains allowed when financial fields are locked. Ordinary Expense OCC/revision still applies.
- An active current Household member may read and append a plain-text comment to a live Expense in that same Household. Each comment stores only comment id, Household id, Expense id, author user id, trimmed body, and trusted server `createdAt`; browser-supplied author, Household, or time is never authoritative. Leaders have no additional comment authority. Soft-deleted Expenses and tombstoned Households retain history but reject new comments.
- Comments are append-only in v1.3, 1–1000 trimmed characters, oldest-to-newest with `createdAt + commentId` stability, and rendered as escaped plain text with preserved newlines. No edit/delete, reactions, mentions, attachments, Markdown/rich text, replies, notification system, or Realtime dependency is introduced.
- Comment creation uses the trusted same-origin/session command path with server-derived actor/time, Appwrite transaction, and actor/type/command-id/SHA-256-intent idempotency. Same key/same body replays the original success; changed intent is rejected. The durable Comment row is the history, so normal creation adds no Household audit event and logs/outcomes never carry the body.
- Comments never update Expense revision/timestamps, the Household financial guard, financial fingerprint, balances, recommendations, or Settlements. Comment counts are derived from Comment rows, never persisted on Expenses. The visible Expenses projection performs one bounded batched count read for its visible IDs, displays Comments immediately before Date on desktop and in mobile metadata before the date, and Expense Details provides the count, authorized current Profile avatar/initials, author Display Name without email, timestamp, body, and explicit-send textarea.
- Schema V7 is additive only: optional `expenses.iconCategory`; new `expense_comments` fields `householdId`, `expenseId`, `authorUserId`, `body`, `createdAt`; and one `expenseId + createdAt` query index. IndexedDB advances monotonically with an Expense-comments store/index and does not rewrite historical Expenses or financial values. Icons use the existing application icon set; comments use TablesDB; no Storage bucket, database, Function, persisted counter, cache, or Realtime channel is added.

## v1.3 Derived Expense settlement status

- The Expenses page includes a read-only, derived Settlement status. `Settled` means the Expense is inside the existing same-Household Confirmed-Settlement financial-history boundary: `latestConfirmedSettlementAt` exists and `expense.createdAt <= latestConfirmedSettlementAt`. `Unsettled` means it is outside that boundary.
- Settlement status uses the same `isExpenseFinanciallyLocked` policy as financial-lock classification. Pending, Rejected, and Cancelled Settlements never establish the status; Expense Date, current balance/recommendation, or the existence of any Settlement alone never establish it. A backdated Expense created after a confirmation remains Unsettled when the existing lock rule leaves it financially editable.
- The status is a derived, read-only presentation field on the existing Expense projection. It is never persisted on Expenses or Settlements, is not a per-Expense linkage or payment record, and requires no Schema V8, IndexedDB migration, backfill, cache, counter, or lifecycle change. The existing bounded Household Settlement read is reused, so this feature adds zero provider reads.
- Desktop displays a compact accessible glyph in a `Settlement` column immediately before `Comments`, preserving `Comments` before `Date`. Mobile displays the same glyph in each existing metadata row before comment count and date. Expense Details already communicates its broader financial-lock state and is not redesigned for this indicator.

## v1.3 Receipt read-authority correction

- **REQUIREMENT CHANGE approved on 2026-09-04:** this section supersedes every active statement that limits Receipt metadata/content reads to the Expense creator or historical uploader. Every current active member of the Expense Household may list the product-safe metadata and open/preview binary content for every `available` Receipt attached to that Expense. The Expense creator and Leader have the same read authority as any other active member. Former members, unrelated users, and anonymous users have no live access.
- Receipt authorization follows the trusted HttpOnly session to server-derived actor, Receipt metadata, Expense, authoritative Expense Household, active membership, `available` state, and private Storage content. Client Household/role/uploader claims, Leader status alone, guessed IDs, direct Storage URLs, and browser Storage credentials are never authority. `user-deleted` and `retention-expired` rows retain historical metadata but never return binary content.
- The member-facing Receipt projection contains only the fields required by the existing UI: `receiptId`, optional `originalFilename`, `mimeType`, `sizeBytes`, `createdAt`, `contentStatus`, and explicit `canReadReceipt`/`canRemoveReceipt` capability state. The Expense projection separately exposes `canReadReceipt`, `canUploadReceipt`, and `canRemoveReceipt`. Storage file IDs/URLs, checksums, reservation/quota internals, command outcomes, provider permissions/errors, uploader identity, and other raw provider fields remain private.
- Receipt upload and removal remain Expense-creator-only. Leader status grants no Receipt mutation authority. Available content may be added/removed only through the existing trusted, private Storage saga; all existing three-per-Expense, 10 MiB/file, 50 MiB/uploader, project-budget, idempotency, retention, resource-isolation, and maintenance behavior is unchanged. This correction requires no schema change beyond the existing zero-drift Schema V7.
...
- **REQUIREMENT CHANGE approved on 2026-08-31:** `profiles.displayName` remains the authoritative product Display Name and is editable from Profile as a mutation separate from password change. Input is a string, trimmed, and must contain 1–20 characters. The 20-character product maximum is enforced independently by the Profile UI, request validation, application boundary, and authoritative persistence path without silent truncation; Appwrite's 16,383-character column capacity remains infrastructure-only. The command derives its actor exclusively from `hft_session`, uses `Profile.version` OCC plus actor/type/command-id/SHA-256 intent idempotency, and treats an unchanged trimmed value as a write-free, version-stable no-op. Appwrite Auth name is not authoritative. Household-name rules are unchanged.
- A Display Name change preserves `Auth userId <-> Profile.userId`, Auth/Profile email, allowlist state, sessions, Household identity/membership/roles, ownership, history, and every financial record/calculation. It does not require a password or log the user out. Current Profile-derived identity projections reconstruct with the new name; deliberately stored historical snapshots remain unchanged. Other authorized Household members receive only the current `{ userId, displayName }` identity projection, never email, Auth metadata, Profile version, or private account fields.
...
- Active Members list Leader first, then other active Members by display-name code-point order and stable member-ID tie-break. Duplicate Profile Display Names are valid within the approved trimmed 1–20-character Profile rule; display name is never identity. Former members are retained but absent from the active list.
...
- Household management never weakens former-member Expense fingerprints or Card privacy. Leaders receive no private Card metadata, private historical Card snapshots, broad raw audits, or persistence records. Receipt access is the narrow exception defined by the v1.3 active-Household read rule; it grants no mutation authority.
...
- **Superseded on 2026-09-04:** Receipt reads now follow the v1.3 active-Household rule above. Every active member may read the UI-safe metadata and `available` content of Receipts attached to Expenses in their Household; upload/removal remains Expense-creator-only, and guessed/foreign identifiers still collapse privacy-safely.
...
## v1.2 Expense Date Window and Profile Pictures

**REQUIREMENT CHANGE approved on 2026-09-01 by product owner request.** This section supersedes the older rule that only future Expense Dates are bounded and that same-date financial edits of a persisted future record must repair its date.

- On Create, Expense Date must be on/after the first day of the current `Asia/Dhaka` calendar month minus exactly two calendar months and on/before the current Dhaka business date. This is calendar arithmetic, never a 60/90-day approximation. The server derives both bounds; browser values are UX only. `EXPENSE_DATE_OUTSIDE_ALLOWED_WINDOW` is the stable lower-bound error; future dates retain `EXPENSE_DATE_IN_FUTURE`.
- Edit applies the window only when the proposed Expense Date differs from the persisted date. Historical Expenses remain stored, visible, reportable, and subject to existing OCC, Confirmed-Settlement, former-member, legacy-percentage, permission, and privacy rules. An unchanged old date does not by itself block a name or otherwise permitted mutation. Validation retains stronger OCC/financial-history locks before the date window and backdated-settlement confirmation after it.
- The styled Date picker keeps unavailable dates visible but semantically disabled, constrains month navigation to months containing a selectable day, and uses server-returned Dhaka bounds only as presentation guidance.
- Each user may have one authoritative private Profile Picture: JPEG, PNG, or WebP, from 1 through 5,242,880 bytes. A Profile Picture is not a Receipt, is excluded from Receipt count/quota/retention/reconciliation, and shares the existing one private Storage bucket under an explicit `avatar_` resource namespace; Receipt files use `receipt_`.
- Schema V6 is additive only: optional private `profiles.avatarFileId` string capacity 64, optional `profiles.avatarUpdatedAt` datetime, and metadata version 6. Neither field enters normal Profile/member browser JSON. The live schema must not change until the dry-run shows only these fields and the metadata bump.
- Same-origin avatar content access derives the actor from the session. A user may read their own avatar; another user may read it only while both have an active membership in the same Household. Unrelated/former access and guessed identities fail privacy-safely. Only the owner can replace their avatar; leadership adds no mutation authority. No public Storage URL, Storage ID, original filename, checksum, credential, or delete authority reaches the browser.
- Replacement validates and fully decodes the new image before upload, uploads before changing the Profile pointer, updates the pointer with Profile-version OCC and command idempotency, then best-effort deletes the old file. A failed pointer update cleans the new file where safe; a failed old-file deletion leaves the new pointer authoritative. Unreferenced `avatar_` files are maintenance-eligible only after a 24-hour grace period and never under Receipt retention.
- Profile shows a circular private image with initials fallback and a keyboard-accessible Change Photo flow. Preview object URLs are revoked on reselection, success, cancel/reset, navigation/unmount, and identity change. Existing avatar slots may use the authorized endpoint without redesigning their screens.
- Backup format includes one verified `avatars/<userId>.bin` per authoritative Profile pointer with provider size, SHA-256, and full image decode checks. Restore writes and verifies the private binary before restoring its Profile pointer. The steady-state avatar envelope is at most approximately 15 MiB for the three approved users; Receipt quotas remain unchanged and Receipt-only.
