...
## 2026-08-30 - Profile Display Name mutation
...
## 2026-09-02 - Expense metadata and append-only child aggregates

- A semantic category belongs in domain vocabulary while icon components belong in one presentation mapping. Optional persistence plus read-time normalization gives legacy compatibility without a write-heavy backfill; the financial fingerprint must remain an explicit allowlist so new metadata cannot silently become financial identity.
- Conversation records are independent child aggregates, not fields on Expense. Creating one should transact only the child row and idempotency outcome: no Expense revision/timestamp, financial guard, audit event, or derived counter write. That separation lets concurrent comments survive and settled history remain byte/value-equivalent.
- For the current bounded list, Appwrite's live-verified array-valued equality query can fetch comments for all visible Expense ids in one read; counts are then derived in memory. Keep stable detail ordering provider-side with `expenseId + createdAt` plus comment id as the tie-break, and resist counters/caches until measured scale requires them.
- Operational backup/reset inventories must advance with every additive table even when the table initially has zero rows. A post-schema backup is the quickest proof that the new table participates in recovery rather than existing only in the application runtime.

## 2026-09-04 - Receipt read authority and compact semantic controls

- Receipt read, upload, and removal are separate capabilities. Household-wide read access should authorize from the Receipt through its Expense to the Expense's authoritative Household and current active membership; uploader identity and Leader role are neither read prerequisites nor mutation grants. Binary delivery adds an independent `available`-state gate and keeps Storage identity server-only.
- An icon-only semantic radio group still needs durable text semantics: ordered accessible names, checked state, visible non-color-only selection, focus indication, and optional focus/hover tooltips. Keep touch targets large while glyphs remain visually subordinate; at narrow widths, a fixed-size non-wrapping row belongs in an internally scrollable strip rather than shrinking or expanding the page.

## 2026-09-12 - In-app notification center

- Notification rows must be produced inside the existing trusted business mutation transaction, after the authoritative business checks and alongside the command outcome. A deterministic row id keyed by event/command, recipient, and notification type makes lost-response retries safe without introducing a second event bus.
- IndexedDB cannot use a nullable `readAt` value as a reliable compound-key component. Keep `readAt` as the sole authoritative state, use a recipient-oriented lookup index, and apply the null/time filters in the provider-independent repository.
- Retention is a calendar policy, not a duration: calculate the first day of the current Dhaka month minus two calendar months once in the domain and reproduce that exact UTC boundary in maintenance. Read filtering remains authoritative while physical cleanup is resumable.
- Household notifications are cached context, not permission. Always query by the server-derived recipient first, then suppress household-scoped rows for former members while allowing minimal account-scoped membership-change rows to remain visible. Store an existing safe parent entity id for comment/receipt deep links instead of leaking child/provider identifiers.
- An explicit recipient rule deserves its own policy function even when it currently equals a broad audience: comment notifications add the authoritative active Expense creator first, then other active members except the commenter, through a Set. Mark-all read state is a naturally idempotent mutation, but still needs a hard provider batch ceiling, one server timestamp per request, and a retryable partial result so a 100-operation transaction limit never becomes a hidden scalability assumption.
