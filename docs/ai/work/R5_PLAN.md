...
## Production execution evidence — in progress

- The feature-branch Site remains at `https://house-finance-tracker.appwrite.network`. Deployment `6a92a22a40f0664b447d` is active from GitHub commit `7ab79b4`; the previous healthy deployment remains ready for rollback. GitHub CI passed for that exact commit.
- Deployed Receipt acceptance passed for 1 MiB, 5 MiB, and exactly 10,485,760 bytes. The exact-boundary JPEG completed through the trusted Route Handler in about 8.35 seconds, survived reload, and rendered through the private content proxy. The over-limit 10,485,761-byte fixture was rejected before consuming a slot or quota.
- Production QA exposed a client orchestration defect: simultaneous Receipt sagas contended on transaction-protected quota guards. Commit `7ab79b4` serializes removals first and then additions while retaining stable command IDs, best-effort continuation, and partial-success accounting. Its focused concurrency regression proves one maximum active upload; the local gate is 710/710 Vitest with lint, TypeScript, build, architecture, audit, and GitHub CI green.
- The three large QA binaries were removed through the normal creator workflow. A verified external backup at `C:\Users\raiya\hft-backups\hft-backup-2026-08-29T09-02-53.742Z` proved zero Storage binaries, retained `user-deleted` metadata with `contentRemovedAt` and `contentRemovedByUserId`, exactly one removal outcome and audit event per Receipt, and zero Expense/uploader/project counters.
- Fixture self-validation found and corrected two test-data defects: the earlier WebP had readable metadata but invalid pixels, and the earlier nominally truncated PNG still decoded. The generator now executes a full sharp pixel decode for accepted fixtures and asserts decode failure for malformed/truncated fixtures. Deployed small JPEG, PNG, and corrected WebP uploads and private previews pass; oversized, malformed, and truly truncated fixtures produce sanitized rejection without metadata creation.
- A verified external snapshot at `C:\Users\raiya\hft-backups\hft-backup-2026-08-29T14-08-30.956Z` contains exactly three available small Receipt binaries totaling 474 bytes with checksum correspondence. Receipt counters are exactly three for the QA Expense, zero for the settled Expense, 474 uploader bytes, and 474 project bytes; rejected reservations are released and create no Receipt metadata.
