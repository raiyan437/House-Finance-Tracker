...
1 production Household · maximum 4 active members (Leader included) · maximum 4 allowlisted production users · approximately 15 Expenses/month (<=180/year) · private in-house use; public SaaS unsupported. Priorities: security, correctness, simplicity, zero cost, maintainability.
...
- **Decision B:** Closed registration. Server-side email allowlist (`APPWRITE_REGISTRATION_ALLOWLIST`), maximum 4 normalized unique emails. Fail-closed: missing/blank/malformed/>4 entries disable registration entirely; empty allowlist can never mean open registration. Allowlist is never exposed to the browser.
...
- Auth user.email is the single authoritative login email; `profiles` stores display name/version/timestamps only. Email changes must flow through the trusted Appwrite Auth workflow in 13B (password reverification surfaced honestly); no independently writable mirrored email.
...
Proportionate fixed-window throttles stored as opaque HMAC-SHA256-derived guard keys (never raw emails/IPs): login 10/15min, recovery 3/h, house-code lookup 10/h, registration 5/day. Completed `command_outcomes` retained indefinitely; DB-only commands commit business mutation + audit + outcome in one transaction; upload saga uses explicit reservation states. All lifecycle timestamps are server-created via the injected Clock in local/dev and the trusted command instant in production; Expense Date remains the validated user business date.
...
| `APPWRITE_REGISTRATION_ALLOWLIST` | server-only configuration | Closed registration; fail-closed, ≤4 emails |
